...
def aggregate_tile(
    ...
    output: Path,
    band_chunk: int = 64,
    pca_model: Path | None = None,
) -> None:
    pca = None
    if pca_model is not None:
        with pca_model.open("rb") as handle:
    ...
    with rasterio.open(tile) as src:
        if pca is not None and getattr(pca, "n_features_in_", src.count) != src.count:
            raise ValueError(
                f"PCA model expects {pca.n_features_in_} bands, but {tile} has {src.count}"
            )

        output_dims = pca.n_components_ if pca is not None else src.count
        ...
        for _, tract in tracts.iterrows():
            ...
            valid_reference=None; count=0

            if pca is None:
                row_sum=np.zeros(output_dims,np.float64); row_sumsq=np.zeros(output_dims,np.float64)
                row_min=np.full(output_dims,np.inf,np.float32); row_max=np.full(output_dims,-np.inf,np.float32)
                ...
                for start in range(1,src.count+1,band_chunk):
                    ...
                    if valid_reference is None:
                        valid_reference=valid; count=int(valid.sum())
                        projected=np.zeros((output_dims,count),np.float64)
                    elif not np.array_equal(valid_reference,valid):
                        raise ValueError("band-dependent nodata is not supported")
                    if count:
                        sl=slice(start-1,start-1+len(idx))
                        centered=values[:,valid_reference].astype(np.float32)-mean[sl,None]
                        projected += components[:,sl] @ centered.astype(np.float64)
                if count:
                    row_sum=projected.sum(1); row_sumsq=np.square(projected).sum(1)
                    row_min=projected.min(1).astype(np.float32); row_max=projected.max(1).astype(np.float32)
    ...
    with temporary.open("wb") as handle:
        np.savez_compressed(handle,GEOID=np.asarray(geoids),count=np.asarray(counts,np.int64),sum=np.asarray(sums),sumsq=np.asarray(sumsqs),min=np.asarray(minima),max=np.asarray(maxima))
...
def main() -> None:
    ...
    a=sub.add_parser("partial"); a.add_argument("--tile",type=Path,required=True); a.add_argument("--tracts",type=Path,required=True); a.add_argument("--output",type=Path,required=True); a.add_argument("--band-chunk",type=int,default=64); a.add_argument("--pca-model",type=Path)
    ...
    x=p.parse_args()
    if x.command=="partial": aggregate_tile(x.tile,x.tracts,x.output,x.band_chunk,x.pca_model)
