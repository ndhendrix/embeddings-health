#!/bin/bash
# Submit corrected OlmoEarth Base overlap inference in bounded rolling waves.
#
# Each invocation scans real outputs, submits a bounded wave of missing tiles,
# then schedules a lightweight controller to refill once the active Base array
...
INFER_JOB_NAME="oe-base-overlap"
...
if [[ "${DRY_RUN:-0}" == 1 || "${CONFIRM_OE_BASE_SUBMIT:-0}" != 1 ]]; then
  available="$MAX_SUBMIT_PER_RUN"
else
  if [[ -n "${WATCH_JOB_ID:-}" ]]; then
    echo "Watching Base GPU work after wave $WATCH_JOB_ID; refill threshold=$REFILL_THRESHOLD"
    while true; do
      ...
      echo "Active Base tasks: $active; total submitted jobs: $total_submitted; refill capacity: $available; checking again in ${REFILL_POLL_SECONDS}s"
      sleep "$REFILL_POLL_SECONDS"
    done
    echo "Refilling with up to $available tasks; $active older Base tasks and $total_submitted total jobs remain active"
    unset WATCH_JOB_ID
  else
    total_submitted=$(squeue -r --noheader --user="$USER" --format='%i' | wc -l)
    available=$((GPU_REQUEST_LIMIT - total_submitted - CONTROLLER_RESERVE))
...
echo "Planned Base wave: $count tasks ($total_missing missing overall); manifest: $MANIFEST"

if [[ "${DRY_RUN:-0}" == 1 || "${CONFIRM_OE_BASE_SUBMIT:-0}" != 1 ]]; then
  echo "DRY RUN (set CONFIRM_OE_BASE_SUBMIT=1 to submit)"
  sed -n '1,20p' "$TASK_FILE"
  exit 0
fi

export REPO_DIR SCRATCH YEAR MODEL OUTPUT_ROOT OVERLAP_OUTPUT_ROOT="$OUTPUT_ROOT" COMPOSITE_DIR CACHE_ROOT BATCH_SIZE
...
if (( count == 0 )); then
  ...
  if (( reduced_missing == 0 )); then
    echo "All Base tiles and state tract reductions are complete for STATE_SET=$STATE_SET."
    exit 0
  fi

  final_partials=$(TASK_FILE="$ALL_TASK_FILE" sbatch \
    --parsable \
    --job-name=oe-base-final-partials \
    --export=ALL \
    --output="$LOG_DIR/oe_base_final_partials_%j.out" \
    --error="$LOG_DIR/oe_base_final_partials_%j.err" \
    "$SCRIPT_DIR/aggregate_missing_partials_driver.sbatch")
  final_partials="${final_partials%%;*}"
  reduction=$(STATE_FILE="$STATE_FILE" sbatch \
    --parsable \
    --job-name=oe-base-reduce \
    --dependency="afterok:$final_partials" \
    --export=ALL \
    --output="$LOG_DIR/oe_base_reduce_%j.out" \
    --error="$LOG_DIR/oe_base_reduce_%j.err" \
...
infer=$(TASK_FILE="$TASK_FILE" sbatch \
  ...
  --output="$LOG_DIR/oe_base_overlap_%A_%a.out" \
  --error="$LOG_DIR/oe_base_overlap_%A_%a.err" \
  "$SCRIPT_DIR/run_overlap_tile.sbatch")
infer="${infer%%;*}"

partials=$(TASK_FILE="$TASK_FILE" sbatch \
  --parsable \
  --job-name=oe-base-partials \
  --dependency="afterany:$infer" \
  --export=ALL \
  --output="$LOG_DIR/oe_base_partials_%j.out" \
  --error="$LOG_DIR/oe_base_partials_%j.err" \
  "$SCRIPT_DIR/aggregate_missing_partials_driver.sbatch")
partials="${partials%%;*}"

continuation=$(sbatch \
  --parsable \
  --job-name=oe-base-overlap-resubmit \
  --partition=normal \
  --time="$CONTROLLER_TIME" \
  --mem=4G \
  --cpus-per-task=1 \
  --output="$LOG_DIR/oe_base_overlap_resubmit_%j.out" \
  --error="$LOG_DIR/oe_base_overlap_resubmit_%j.err" \
  --export=ALL,WATCH_JOB_ID="$infer" \
  --wrap="bash '$SCRIPT_DIR/submit_olmoearth_base_overlap_resubmit.sh'")
continuation="${continuation%%;*}"
echo "Submitted Base inference=$infer partials=$partials continuation=$continuation"
