...
INFLIGHT_STATES_FILE="$CACHE_ROOT/clay_merge_inflight_states.txt"
...
# tile tasks to depend on instead (Clay is merge-only, so this is the normal
# path for it), so the resubmit doesn't tight-loop with nothing new to wait on.
